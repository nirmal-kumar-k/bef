...
const navGroups = [
  {
    ...
    items: [
      ...
      { href: '/orders', label: 'Orders', badge: '12', icon: <ListDashes weight="duotone" className="w-[18px] h-[18px] opacity-80 group-hover:opacity-100 transition-opacity" /> },
    ]
  },
  {
    label: 'Master Data',
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      >
        {isOpen ? <X weight="duotone" size={20} /> : <List weight="duotone" size={20} />}
      </button>
      ...
      >
        ...
        <div className="flex items-center gap-[14px] px-5 h-[64px] border-b border-[#243050] shrink-0">
          <svg className="w-[36px] h-[36px] shrink-0 relative" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M13 15H20C21.66 15 23 16.34 23 18C23 19.66 21.66 21 20 21H13V15Z" fill="#EB6824"/>
            <rect x="10" y="27" width="16" height="2.5" rx="1.25" fill="#9B3A0C"/>
            <circle cx="29" cy="7" r="1.5" fill="#D4521A" opacity="0.7"/>
            <circle cx="27" cy="4" r="1" fill="#EB6824" opacity="0.5"/>
                ...
                return (
                  <Link
                    ...
                    className={`group flex items-center gap-[11px] px-[12px] py-[9px] rounded-[10px] text-[13.5px] transition-all border border-transparent select-none tracking-[-0.1px] ${
                      active
                        ? 'bg-[#D4521A]/20 border-[#D4521A]/35 text-[#EB6824] font-medium'
                        : 'text-[#8B9FC4] font-normal hover:bg-[#1C2840] hover:text-[#EEF3FF]'
                    }`}
                  >
                    <div className={`${active ? 'text-[#EB6824]' : 'text-[#8B9FC4] group-hover:text-[#EEF3FF]'} transition-colors`}>
                      {item.icon}
                    </div>
                    {item.label}
                    {item.badge && (
