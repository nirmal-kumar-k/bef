...
export function MeltPlanningTab({ defaultMetalQty, defaultGrade }: { defaultMetalQty: number, defaultGrade?: string }) {
  // Master State
                    ...
                    <table className="w-full text-sm text-left bg-[#050810]">
                      <thead className="text-[#8B9FC4] text-xs uppercase bg-[#1A263D]/30">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-right">Melt Wt (kg)</th>
                        ...
                        {dayHeats.map(h => {
                          ...
                          return (
                            <tr key={h.heatNo} className="hover:bg-[#1A263D]/50 transition-colors">
                              ...
                              <td className="px-4 py-2 font-mono text-[#D4521A] text-right">{h.meltWeight}</td>
                    ...
                    <div className="p-4 bg-[#050810] flex-1">
                       <table className="w-full text-xs text-left">
                        <thead className="text-[#5A6E90] uppercase border-b border-[#243050]">
                          <tr>
                            <th className="py-1">Heat</th>
                            <th className="py-1">Slot</th>
                            <th className="py-1 text-right">Wt (kg)</th>
                            <th className="py-1 text-right">Pig Iron</th>
                            <th className="py-1 text-right">Scrap</th>
                        ...
                        <tbody className="divide-y divide-[#243050]/50">
                          {dayHeats.map(h => {
                            ...
                            return (
                              <tr key={h.heatNo}>
                                <td className="py-2 font-mono text-[#EEF3FF]">{h.heatNo}</td>
                                <td className="py-2 text-[#8B9FC4]">{h.slot}</td>
                                <td className="py-2 font-mono text-[#D4521A] text-right">{h.meltWeight}</td>
                                <td className="py-2 text-right text-[#8B9FC4]">{(h.meltWeight * rec.pigIron / 100).toFixed(1)}</td>
                                <td className="py-2 text-right text-[#8B9FC4]">{(h.meltWeight * rec.scrap / 100).toFixed(1)}</td>
        ...
        <div className="space-y-6">
          <div className="bg-[#0C1221] p-6 rounded-xl border border-[#243050]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Required Qty (kg)</Label>
                <Input type="number" value={rmQty} onChange={e => setRmQty(Number(e.target.value))} className="bg-[#050810] border-[#243050] text-[#EEF3FF]" disabled={rmHeatView !== 'all'} />
              </div>
              <div className="md:col-span-2">
                <Label className="text-[#8B9FC4] text-xs mb-1 block">View Specific Heat</Label>
                <Select value={rmHeatView} onValueChange={(v) => { 
                  ...
                  <SelectContent className="bg-[#0C1221] border-[#243050] text-[#EEF3FF]">
                    ...
                    {heats.map(h => <SelectItem key={h.heatNo} value={h.heatNo}>{h.heatNo} - {h.meltWeight}kg</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-[#8B9FC4] text-xs uppercase bg-[#1A263D]/30 border-b border-[#243050]">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-right">Required (kg)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#243050]">
                    <tr>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-[#D4521A]">{(rmQty * rec.pigIron / 100).toFixed(1)}</td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 font-medium text-[#EEF3FF]">Scrap</td>
                      <td className="px-6 py-4 text-center text-[#8B9FC4]">{rec.scrap}%</td>
                      <td className="px-6 py-4 text-right font-mono text-[#D4521A]">{(rmQty * rec.scrap / 100).toFixed(1)}</td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 font-medium text-[#EEF3FF]">FeMn</td>
                      <td className="px-6 py-4 text-center text-[#8B9FC4]">{rec.feMn}%</td>
                      <td className="px-6 py-4 text-right font-mono text-[#D4521A]">{(rmQty * rec.feMn / 100).toFixed(1)}</td>
                    </tr>
                    <tr>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-[#D4521A]">{(rmQty * rec.carburizer / 100).toFixed(1)}</td>
                    </tr>
                  </tbody>
                  <tfoot className="bg-[#0C1221] border-t border-[#243050]">
                    <tr>
                      ...
                      <td className="px-6 py-4 text-right font-mono font-bold text-[#D4521A] text-lg">{rmQty.toFixed(1)}</td>
                    </tr>
                  </tfoot>
                </table>
              )
      ...
      {activeSection === 'meltingNotes' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
             <p className="text-[#8B9FC4] text-xs font-mono">Record heat-wise observations. Linked to planned heats for full traceability.</p>
             {heats.length > 0 && (
               <div className="flex gap-4">
                 <div className="text-center"><p className="text-[10px] text-[#8B9FC4] uppercase">Planned</p><p className="font-bold text-[#EEF3FF]">{heats.length}</p></div>
                 <div className="text-center"><p className="text-[10px] text-[#8B9FC4] uppercase">Recorded</p><p className="font-bold text-[#4285F4]">{notes.length}</p></div>
                 <div className="text-center"><p className="text-[10px] text-[#8B9FC4] uppercase">Pending</p><p className="font-bold text-amber-500">{heats.length - notes.length}</p></div>
               </div>
             )}
          </div>

          <div className="bg-[#0C1221] border border-[#243050] rounded-xl p-6">
            <h4 className="font-bold text-[#EEF3FF] mb-4">Add / Edit Melting Note</h4>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div className="md:col-span-2">
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Planned Heat Link (Optional)</Label>
                <Select value={noteSelectedHeat} onValueChange={(v) => {
                  ...
                  <SelectTrigger className="bg-[#050810] border-[#243050] text-[#EEF3FF]"><SelectValue placeholder="Select from plan..."/></SelectTrigger>
                  <SelectContent className="bg-[#0C1221] border-[#243050] text-[#EEF3FF]">
              ...
              <div className="md:col-span-2">
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Heat No (Manual)</Label>
                <Input value={noteForm.heatNo || ''} onChange={e => setNoteForm({...noteForm, heatNo: e.target.value})} placeholder="Or type manually..." className="bg-[#050810] border-[#243050] text-[#EEF3FF]" disabled={!!noteSelectedHeat} />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 pb-6 border-b border-[#243050]">
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Grade</Label>
              ...
              <div>
                ...
                <Input type="number" value={noteForm.meltWeight || ''} onChange={e => setNoteForm({...noteForm, meltWeight: Number(e.target.value)})} className="bg-[#050810] border-[#243050] text-[#EEF3FF]" />
              </div>
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Temperature (°C)</Label>
                <Input type="number" value={noteForm.temp || ''} onChange={e => setNoteForm({...noteForm, temp: Number(e.target.value)})} className="bg-[#050810] border-[#243050] text-[#EEF3FF]" />
              </div>
            </div>

            <h5 className="text-[#8B9FC4] text-xs font-semibold mb-3">COMPOSITION (%)</h5>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Carbon (C)</Label>
                <Input type="number" step={0.01} value={noteForm.c ?? ''} onChange={e => setNoteForm({...noteForm, c: e.target.value ? Number(e.target.value) : null})} className="bg-[#050810] border-[#243050] text-[#EEF3FF] font-mono" />
              </div>
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Silicon (Si)</Label>
                <Input type="number" step={0.01} value={noteForm.si ?? ''} onChange={e => setNoteForm({...noteForm, si: e.target.value ? Number(e.target.value) : null})} className="bg-[#050810] border-[#243050] text-[#EEF3FF] font-mono" />
              </div>
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Manganese (Mn)</Label>
                <Input type="number" step={0.01} value={noteForm.mn ?? ''} onChange={e => setNoteForm({...noteForm, mn: e.target.value ? Number(e.target.value) : null})} className="bg-[#050810] border-[#243050] text-[#EEF3FF] font-mono" />
              </div>
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Phosphorus (P)</Label>
                <Input type="number" step={0.001} value={noteForm.p ?? ''} onChange={e => setNoteForm({...noteForm, p: e.target.value ? Number(e.target.value) : null})} className="bg-[#050810] border-[#243050] text-[#EEF3FF] font-mono" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div>
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Sulphur (S)</Label>
                <Input type="number" step={0.001} value={noteForm.s ?? ''} onChange={e => setNoteForm({...noteForm, s: e.target.value ? Number(e.target.value) : null})} className="bg-[#050810] border-[#243050] text-[#EEF3FF] font-mono" />
              </div>
              <div className="md:col-span-3">
                <Label className="text-[#8B9FC4] text-xs mb-1 block">Remarks</Label>
                <Input value={noteForm.remarks || ''} onChange={e => setNoteForm({...noteForm, remarks: e.target.value})} className="bg-[#050810] border-[#243050] text-[#EEF3FF]" />
              </div>
            </div>
