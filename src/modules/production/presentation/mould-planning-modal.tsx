  ...
  useEffect(() => {
    ...
    setHourlyMatrix(prevMatrix => {
      ...
      activeMoulds.forEach(cb => {
        ...
        TIME_SLOTS.forEach(slot => {
          machineMatrix[slot.time] = avgProd
        })
    ...
    setWorkers(prevWorkers => {
      ...
      activeMoulds.forEach(cb => {
        ...
        if (Object.values(w).some(v => v !== undefined && v > 0)) {
          hasExistingWorkers = true
        }
      })
      ...
      activeMoulds.forEach(cb => {
        ...
        TIME_SLOTS.forEach(slot => {
          machineWorkers[slot.time] = 1 // Equipment assignment = 1
        })
  ...
  const handleWorkerChange = (patternRef: string, timeSlot: string, delta: number) => {
    ...
    setWorkers(prev => {
      ...
      if (assignmentType === 'equipment') {
        ...
        setHourlyMatrix(prevMatrix => {
          const matrixKey = prevMatrix[key] || {}
          return {
            ...prevMatrix,
            [key]: {
               ...matrixKey,
               [timeSlot]: next > 0 ? (next * avgProd) : undefined
            }
          }
        })
      }
          ...
          {selectedOrder && activeMoulds.length > 0 && activeMachineTab && (
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2 bg-[#F4F6FB] p-1 rounded-lg border border-[#E0E7FF]">
                  <button 
                    onClick={() => setAssignmentType('equipment')}
                    className={cn("px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-colors", assignmentType === 'equipment' ? "bg-[#FFFFFF] text-[#4285F4] shadow-sm" : "text-[#94A3B8] hover:text-[#172554]")}
                  >
                    Equipment
                  </button>
                  <button 
                    onClick={() => setAssignmentType('labourer')}
                    className={cn("px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-colors", assignmentType === 'labourer' ? "bg-[#FFFFFF] text-[#4285F4] shadow-sm" : "text-[#94A3B8] hover:text-[#172554]")}
                  >
                    Labourer
                  </button>
                </div>
                  ...
                  <thead className="bg-[#FFFFFF] border-b border-[#E0E7FF] text-[#64748B] uppercase tracking-wider font-bold">
                    ...
                    <tr>
                      {activeMoulds.map(cb => [
                        ...
                        <th key={`${cb.patternRef}-assign`} className="px-2 py-2 text-center text-[10px] text-[#94A3B8] w-56 font-medium">
                          {assignmentType === 'equipment' ? 'EQUIPMENT ASSIGN.' : 'LABOURER ASSIGN.'}
                        </th>
                      ])}
                    </tr>
                  </thead>
