...
export function KnockoutPlanningTab({ knockoutBacklog, patterns, openOrders, dailyPlans, onSaveDayPlan }: KnockoutPlanningTabProps) {
  ...
  const totalRemainingKnockouts = knockoutBacklog.reduce((sum, b) => sum + Math.max(0, b.totalRequired - b.totalScheduled), 0)
  
  // Calculate average capacity per shift across all required patterns
  const averageKnockoutsPerHour = useMemo(() => {
    let sum = 0
    let count = 0
    knockoutBacklog.forEach(b => {
      const pattern = patterns.find(p => p.code === b.patternRef)
      if (pattern && pattern.avgKnockoutsPerHour) {
        sum += Number(pattern.avgKnockoutsPerHour)
        count++
      }
    })
    return count > 0 ? sum / count : 20 // default 20 if not set
  }, [knockoutBacklog, patterns])

  const shiftCapacity = averageKnockoutsPerHour * SHIFT_HOURS
  const capacityRatio = totalRemainingKnockouts / shiftCapacity
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-[#F4F6FB] border border-[#E0E7FF] rounded-xl relative flex flex-col p-4 overflow-x-auto">
          ...
          <div className="grid grid-cols-7 gap-3 flex-1 min-w-[800px]">
            {days.map((date, i) => {
              ...
              const pendingAmount = knockoutBacklog.reduce((s, b) => s + Math.max(0, b.totalRequired - b.totalScheduled), 0)
                    ...
                    {sum > 0 && (
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10.5px] font-medium text-[#64748B]">Knockouts</span>
                        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-[#FFFFFF] border-b border-[#E0E7FF] text-[#64748B] text-[11px] uppercase tracking-wider font-bold">
              <tr>
                ...
                <th className="px-6 py-4 text-center">Avg Knockouts/hr</th>
            ...
            <tbody className="divide-y divide-[#E0E7FF] text-[#172554]">
              {knockoutBacklog.map((b) => {
                ...
                const pattern = patterns.find(p => p.code === b.patternRef)
                const avgPerHour = pattern?.avgKnockoutsPerHour || '-'
      ...
      {/* Capacity Indicator & Action */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center w-full">
        <Button onClick={() => setIsConfirmModalOpen(true)} className="bg-[#10B981] hover:bg-[#10B981]/90 text-white font-bold tracking-wider px-6">
          <CheckCircle className="w-5 h-5 mr-2" />
          CONFIRM KNOCKOUTS TO INVENTORY
        </Button>
      </div>
        ...
        <div className="bg-[#FFFFFF] border border-[#E0E7FF] rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[#172554] font-semibold flex items-center gap-2">
                Knockout Line Capacity
              </h3>
              <p className="text-xs text-[#64748B] mt-1">Based on {SHIFT_HOURS} hr shift and avg {averageKnockoutsPerHour.toFixed(1)} moulds/hr</p>
            </div>
            <div className="text-right">
              <span className="text-2xl font-bold font-mono text-[#172554]">{totalRemainingKnockouts}</span>
          ...
          <div className="relative h-4 bg-[#F4F6FB] rounded-full overflow-hidden border border-[#E0E7FF]">
            <div 
              ...
              style={{ width: `${Math.min(100, (totalRemainingKnockouts / (shiftCapacity * 10)) * 100)}%` }}
            />
          </div>
      ...
      <KnockoutConfirmModal
        isOpen={isConfirmModalOpen}
        onClose={() => setIsConfirmModalOpen(false)}
        dailyPlans={dailyPlans}
        patterns={patterns}
        onConfirmSuccess={() => {
          // This should trigger a refetch. By updating a plan, onSaveDayPlan might be useful.
