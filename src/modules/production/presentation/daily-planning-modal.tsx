      ...
      <DialogContent className="w-full sm:max-w-2xl bg-[#F4F6FB] border-[#E0E7FF] text-foreground max-h-[85vh] overflow-y-auto">
        ...
        <div className="py-4 space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-[#64748B] text-xs font-semibold uppercase tracking-wider">Scheduled Today</h3>
            {dailyPlans.length === 0 ? (
              <p className="text-sm text-[#94A3B8] italic">No {stage.toLowerCase()}s scheduled for this day.</p>
            ) : (
              <div className="space-y-2">
                {dailyPlans.map(plan => {
                  const bItem = backlog.find(b => b.itemId === plan.itemId && b.coreBoxCode === plan.coreBoxCode)
                  return (
                    <div key={plan.id} className="flex items-center justify-between bg-[#FFFFFF] border border-[#E0E7FF] rounded-lg p-3">
                      <div>
                        <p className="text-[#172554] font-medium text-sm">
                          {bItem?.orderNo} — {bItem?.productName}
                          {plan.coreBoxCode && <span className="text-[#4F46E5] ml-2 font-mono">[{plan.coreBoxCode}]</span>}
                        </p>
                        <p className="text-[#64748B] text-xs mt-1">Pattern: {bItem?.patternRef}</p>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          <p className="text-[#172554] font-mono text-lg">{plan.quantityScheduled}</p>
                          <p className="text-[#94A3B8] text-[10px] uppercase">Qty</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[#4285F4] font-mono text-lg">{plan.laborersAssigned}</p>
                          <p className="text-[#94A3B8] text-[10px] uppercase">Laborers</p>
                        </div>
          ...
          <hr className="border-[#E0E7FF]" />

          {/* Add new plan */}
          <div className="bg-[#FFFFFF] border border-[#E0E7FF] rounded-lg p-4 space-y-4">
            <h3 className="text-[#172554] font-semibold">Assign New Task</h3>
            
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label className="text-[#64748B]">Select Requirement</Label>
                <Select value={selectedItemId} onValueChange={(val) => {
                  ...
                  <SelectContent className="bg-[#FFFFFF] border-[#E0E7FF]">
                    ...
                    {availableBacklog.map(b => {
                      const uniqueId = stage === 'Core' ? `${b.itemId}::${b.coreBoxCode}` : b.itemId
                      return (
                        <SelectItem key={uniqueId} value={uniqueId} className="text-[#172554]">
                          {b.orderNo} | {b.patternRef} | {b.productName} 
                          {stage === 'Core' && <span className="text-[#4F46E5] ml-1">[{b.coreBoxCode}]</span>}
                          <span className="text-[#64748B] ml-2 text-xs">(Rem: {b.totalRequired - b.totalScheduled})</span>
                        </SelectItem>
                      )
                    })}
                  </SelectContent>
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-1.5">
                    <div className="flex justify-between">
                      <Label className="text-[#64748B]">Quantity</Label>
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-[#64748B]">Laborers Assigned</Label>
                    <Input 
                      type="number" 
                      min="1" 
                      value={laborers}
